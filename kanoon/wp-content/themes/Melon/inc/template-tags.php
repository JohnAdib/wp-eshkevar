...
function web2feel_content_nav( $nav_id ) {
	...
	<?php if ( is_single() ) : // navigation links for single posts ?>
		...
		<?php previous_post_link( '<hr /><div class="nav-previous">%link</div>', '<span class="meta-nav">قبلی &larr; </span>' ); ?>
		<?php next_post_link( '<div class="nav-next">%link</div>', '<span class="meta-nav">&rarr; بعدی</span>' ); ?>

	<?php elseif ( $wp_query->max_num_pages > 1 && ( is_home() || is_archive() || is_search() ) ) : // navigation links for home, archive, and search pages ?>

		<?php if ( get_next_posts_link() ) : ?>
		<div class="nav-previous"><?php next_posts_link( '<span class="meta-nav">مطالب قدیمی تر &larr;</span>'  ); ?></div>
		<?php endif; ?>

		<?php if ( get_previous_posts_link() ) : ?>
		<div class="nav-next"><?php previous_posts_link( '<span class="meta-nav">&rarr; مطالب جدیدتر</span>' ); ?></div>
		<?php endif; ?>
...
function web2feel_posted_on() {
	printf( 'ارسال شده در <a href="%1$s" title="ساعت %2$s" rel="bookmark"><time class="entry-date" datetime="%3$s">%4$s</time></a><span class="byline"> توسط <span class="author vcard"><a class="url fn n" href="%5$s" title="%6$s" rel="author">%7$s</a></span></span>',
		esc_url( get_permalink() ),
		esc_attr( get_the_time() ),
		esc_attr( get_the_date( 'c' ) ),
		esc_html( get_the_date() ),
		esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ),
		esc_attr( sprintf( 'مرور تمام نوشته های %s', get_the_author() ) ),
